...
//      // 5 vs 6 vs 9
        ...
        do{
            do {
                routeMessage(match2,p5, getNextMessageFrom(p5Connection));
            } while (getLastMessage(p5ActualOut).getCode() != Message.Code.END_TURN);
            do {
                routeMessage(match2,p6, getNextMessageFrom(p6Connection));
            } while (getLastMessage(p6ActualOut).getCode() != Message.Code.END_TURN);
            do {
                routeMessage(match2, p9, getNextMessageFrom(p9Connection));
            } while (getLastMessage(p9ActualOut).getCode() != Message.Code.END_TURN);
        }while (p9Connection.readLine()==null);
    ...
    private void routeMessage(Controller c, VirtualView v, Message m) {
        switch (m.getCode()) {
            case CHOOSE_WORKERS_INITIAL_POSITION:
